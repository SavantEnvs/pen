...
# OSS-Fuzz Rust libFuzzer+ASan flags. Rust drives sanitizers via RUSTFLAGS, not the
# base image's clang-oriented $SANITIZER_FLAGS / CFLAGS (rustc ignores those); this
# -Zsanitizer=address is the rustc equivalent. cargo-fuzz sets the ASan flag itself,
# but we pin it explicitly. --cfg fuzzing matches libfuzzer-sys; force-frame-pointers
# aids ASan backtraces.
#
# DWARF < 4 debug info (SPEC §6.2 item 10): cargo-fuzz links librustc-nightly_rt.asan.a
# (DWARF5) via --whole-archive, placing it at .debug_info offset 0. -Zdwarf-version=3
# alone does NOT win because the ASan runtime CU lands first; the cc-wrapper (built in
# mayhem/Dockerfile) injects a DWARF3 anchor.o as the very first linker input.
...
# non-sanitized build) — so mayhem/test.sh only RUNS it. Upstream's unit suite is
# tools/unit_test.sh = `cargo test` in every directory holding a Cargo.lock
# (root workspace + cmd/test + packages/*/ffi); we pre-compile all of them here.
# The root workspace needs LLVM 16 via $LLVM_SYS_160_PREFIX (mir-fmm -> llvm-sys).
echo "=== building upstream test suite (normal flags, no sanitizers) ==="
for d in $(git ls-files 'Cargo.lock' '**/Cargo.lock' | grep -v '^mayhem/'); do
  dir="$(dirname "$d")"
  echo "--- cargo test --no-run in $dir ---"
  ( cd "$dir" && env -u RUSTFLAGS cargo test --no-run )
done
